  ...
  <div class="main-content bg-default">
    <b-container>
      ...
      <b-row cols="12" class="mb-4">
        <b-col sm="6">
          <b-card title="Users" img-alt="Image" img-top tag="article">
            <b-card-body>
              <b-container fluid>
                <ul>
                  <li
                    ...
                    v-for="user in users"
                    :key="user.id"
                    @click="userSelected($event, user.id)">
                    id: {{ user.id }}, name: {{ user.name }} - salary as {{user.salary | convertToSalary | addCurrency}} - {{user.salary}}
                  </li>
                </ul>
              </b-container>
            </b-card-body>
